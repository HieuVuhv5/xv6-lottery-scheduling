...
int nextpid = 1;
static int nice_value_control = 0;
int total_tickets;
...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  struct rtcdate *curr_d = (struct rtcdate *)(kalloc());
  cmostime((struct rtcdate *)curr_d);
  p->begin_date = curr_d;
...
//#endif // PROC_TIMES
#ifdef LOTTERY
//# error this is a good place to set the default nice 
	p->nice_value = DEFAULT_NICE_VALUE;
	/*
if (nice_value_control == 0){
	p->nice_value = DEFAULT_NICE_VALUE;
}
else
{

	p->nice_value = nice_value_control;
...
//# error of that 
	if(nice_value_control == 0){
		...
		np->nice_value = nice_value_control;
	}
#endif // LOTTERY
...
//#endif // LOTTERY
  ...
  for(;;){
	...
	//cprintf("\trandom number is %u\t total nice number is : %u",rand_number,nice_sum);
	
	if( nice_sum > 0 ){
		winner = rand_number % nice_sum + 1;
		nice_sum = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;

	  if(counter + p->nice_value < winner){
	  ...
	  p->sched_time++;
      p->ticks_begin = proc_uptime_ticks();
...
//acquire(&ptable.lock);
	for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
		if(p->state == RUNNABLE){
			nice_tt += p->nice_value;
		}//end if
	}//end for
	//release(&ptable.lock);
	//cprintf("\t total nice number is : %u",nice_tt);
	return nice_tt;
}
...
int
proc_nice(uint value)
{
	nice_value_control = value;
	
	return 0;
}

int
proc_renice(uint nice_val, uint pid_1, uint pid_2)
{
	int count_time = 0;
	struct proc *p;
	if(nice_val < MIN_NICE_VALUE || nice_val > MAX_NICE_VALUE)
	{
		cprintf("The nice value is out of bounds \nThe Nice Value should in : %u to  %u\n",MIN_NICE_VALUE, MAX_NICE_VALUE);
		return 1;
	}
	...
	{
		// NO pid was found 
		cprintf("The PID(s) Not exist\n");
		return 2;
	}
	if( count_time < 2)
	{
		// not of all pid(s) was found.
		cprintf("Not all Pid(s) exist\n");
		return 2;
	}
...
# error this is an excellent place to add some new header into to the o/p of cps
    ...
    for (i = 0; i < NPROC; i++) {
        if (ptable.proc[i].state != UNUSED) {
			...
			//cmostime(ptable.proc[i].begin_date);
			/*
			crr_prt = (struct rtcdate *)ptable.proc[i].begin_date;
			
			uint year_pr =11110;
			if( crr_prt ){
				 year_pr = crr_prt->year;
				
			}
			*/
			struct rtcdate *date_temp = ptable.proc[i].begin_date;
			/*
			if ( ! ptable.proc[i].nice_value)
			{
				ptable.proc[i].nice_value = DEFAULT_NICE_VALUE;
			}
			*/	
            cprintf("%d\t%d\t%s\t%s\t%u\t%u-%s%u-%s%u  %s%u:%s%u:%s%u\t%u\t%u\t%u"
                    , ptable.proc[i].pid
                    ...
                    , ptable.proc[i].sz
					, date_temp->year
					, (date_temp->month < 10) ? "0" : ""
					, date_temp->month
					, (date_temp->day < 10) ? "0" : ""
					, date_temp->day
					, (date_temp->hour < 10) ? "0" : ""
					, date_temp->hour
					, (date_temp->minute < 10) ? "0" : ""
					, date_temp->minute
					, (date_temp->second < 10) ? "0" : ""
					, date_temp->second
					, ptable.proc[i].ticks_total
					, ptable.proc[i].sched_time
...
# error this is an excellent place to add some new data to the o/p of cps
    ...
    release(&ptable.lock);
	// reset nice_value_control
	nice_value_control = 0;
    return 0;
}
#endif // CPS
